...
} from "./SchemaDetails/schemaRecommendationQuery";
    ...
    >
      ...
      <div className="pl-6 pb-5">
        ...
        {saveSchemaMatchError && (
          <p className="text-overline2 text-accent-red my-2">Fill all fields</p>
        )}
      </div>
    ...
    <TableRow>
      ...
      >
        <Select
          ...
          value={
            (unMappedFields
              ? newTableValue["unMappedFields"]
              : newTableValue["mappedFields"]) ?? tableName
          }
          variant="outlined"
          disabled={!canEdit()}
          onChange={(event) => {
      ...
      >
        <Select
          ...
          value={
            (unMappedFields
              ? newColumnValue["unMappedFields"]
              : newColumnValue["mappedFields"]) ?? columnName
          }
          disabled={!canEdit()}
          variant="outlined"
        ...
        >
          {getColumns(
            newTableValue[
              unMappedFields ? ["unMappedFields"] : ["mappedFields"]
            ] ?? tableName
          )?.map((column) => {
            return (
