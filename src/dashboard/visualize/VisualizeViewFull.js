...
const VisualizeViewFull = React.forwardRef((props, ref) => {
  useImperativeHandle(ref, () => ({
    async parseFile(sankey_result,file) {
    setErrorMessage(null);
    let svgElement = document.getElementById("svgd3")
    if (svgElement) svgElement.remove();

    if (sankey_result.data) {
      setParsed(true)
  ...
  useEffect(() => {
    async function generateGraph(sankey_result) {
      setErrorMessage(null);
      let svgElement = document.getElementById("svgd3")
      if (svgElement) svgElement.remove();
    ...
    if (props.sankey_data && props.sankey_data != null) {
      setErrorMessage('');
      generateGraph(props.sankey_data);
    } else {
...
export default VisualizeViewFull;